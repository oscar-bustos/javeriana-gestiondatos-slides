...
- **Variedad (Variety):** Diversidad de estructuras (tabulares, multimedia, grafos libres).
...
## Solución Era 2010s: Cómputo Cerca al Disco

```python
# En un Cluster Hadoop con Nodos distribuyendo 1 Petabyte de Logs:
# NodeData_1 ejecuta este Mapper (su fraccion del trabajo paralela):
def mapper(linea_texto_local):
for log in linea_texto_local.split():
yield (log.categoria, 1)

# Se satura el canal de red trasladando (Shuffle) los
# pares, para ser absorbidos por el Nodo Reductor final:
def reducer(categoria, conteo_parcial):
yield (categoria, sum(conteo_parcial))
```
...
::: {.column width="50%"}
\begin{center}
![](assets/hdfs_blocks.png){width="100%"}
\end{center}
:::
...
\begin{center}
![](assets/diagrama_datalake_madurez.png){width="70%"}
\end{center}
...
::: {.column width="50%"}
\begin{center}
![](assets/columnar_storage.png){width="100%"}
\end{center}
:::
...
## Solución Era 2015s: Streaming Perezoso (Lazy Eval)

```python
# En la actualidad (2020s), una RAM pequena frente a un archivo masivo colapsa con Pandas eager. Solucion In-Memory Perezosa "Modern Polars":
import polars as pl

# POLARS: Carga Virtual Lazy Streaming. Abre flujos en paralelo y jamas
# asimila la totalidad colapsando la RAM. Pone en batch al procesador:
plan = (
...
# Evalua el plan hasta la recoleccion interactiva final
...
\begin{center}
![](assets/diagrama_kappa_lambda.png){width="95%"}
\end{center}
...
::: {.column width="50%"}
\begin{center}
![](assets/medallion_model.png){width="100%"}
\end{center}
:::
...
## Solución Era 2020s+: Serverless Transparente

```sql
-- Google BigQuery (Serverless Distribuido)
-- Sin instalar nada de Python o clusters locativos.
-- Almacena exabytes y distribuye instantaneamente procesadores a peticion:
